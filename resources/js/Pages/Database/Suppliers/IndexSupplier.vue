<script setup>
    ...
    const search = useDebouncedTableSearch('suppliers.index', usePage().props.search);
        ...
        <div class="grid grid-cols-1 h-full space-y-6">
            ...
            <div class="bg-gradient-to-r from-rose-600 to-rose-700 rounded-xl shadow-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-rose-100 mt-1">Kelola data supplier dan pemasok</p>
                    </div>
                    <div class="hidden md:block">
                        <div class="bg-white/20 backdrop-blur-sm rounded-lg p-4">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-12 h-12 text-white">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M20.25 14.15v4.25c0 .414-.336.75-.75.75h-4.5a.75.75 0 0 1-.75-.75v-4.25m0 0h4.5m-4.5 0-3 3m3-3 3-3m0 0v-4.25m0 4.25h-4.5M3.75 9.75h4.5m-4.5 0v4.25m0-4.25-3 3m3-3 3-3m0 0v-4.25m0 4.25h4.5m-4.5 0-3 3m3-3 3-3" />
                            </svg>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-md p-4">
                <div class="flex flex-col md:flex-row justify-between gap-4">
                    <div class="w-full md:w-1/3">
                        <div class="relative group">
                            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-gray-400 group-focus-within:text-rose-600 transition-colors">
                    ...
                    <div>
                        <template v-if="hasPermission('supplier: create')">
                            <button 
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                <Table>
                    <template #header>
                        <TableRow class="bg-rose-500">
                            <TableHeaderCell class="text-white">NO</TableHeaderCell>
                            <TableHeaderCell class="text-white">NAMA SUPPLIER</TableHeaderCell>
                            <TableHeaderCell class="text-white">TELEPON</TableHeaderCell>
                            <TableHeaderCell class="text-white">ALAMAT</TableHeaderCell>
                            <TableHeaderCell class="text-white">AKSI</TableHeaderCell>
                        </TableRow>
                    </template>
                    <template #default>
                        <TableRow v-for="(data, index) in fetchData.data" :key="data.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-rose-50 dark:hover:bg-gray-600 transition-colors duration-150">
                            <TableDataCell :status="'number'">{{ index+1 }}</TableDataCell>
                            <TableDataCell :status="'record'">{{ data.name }}</TableDataCell>
                            <TableDataCell :status="'record'">{{ data.phone }}</TableDataCell>
                            <TableDataCell :status="'record'">{{ data.address }}</TableDataCell>
                            <TableDataCell :status="'action'">
                                <template v-if="hasPermission('supplier: read')">
                                    <button 
                                ...
                                <template v-if="hasPermission('supplier: update')">
                                    <button 
                                ...
                                <template v-if="hasPermission('supplier: delete')">
                                    <button 
