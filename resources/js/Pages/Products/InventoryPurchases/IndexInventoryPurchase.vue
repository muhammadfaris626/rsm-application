                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                        ...
                        <li aria-current="page">
                            <div class="flex items-center">
                                ...
                                <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Pembelian Persediaan</span>
                            </div>
                        </li>
                    </ol>
                </nav>
            ...
            <div class="flex justify-between my-3">
                ...
                <div>
                    <template v-if="hasPermission('inventory-purchase: create')">
                        <Link :href="route('inventoryPurchases.create')" class="px-5 py-2.5 text-sm font-medium text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
            ...
            <Table>
                <template #header>
                    <TableRow>
                        ...
                        <TableHeaderCell>NOMOR FAKTUR</TableHeaderCell>
                ...
                <template #default>
                    <TableRow v-for="(data, index) in fetchData.data" :key="data.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <TableDataCell :status="'record'" class="font-bold">
                            <Link :href="route('inventoryPurchases.show', data.id)" class="text-blue-500 hover:text-blue-600">
                                {{ data.invoice_number }}
                            </Link>
                        </TableDataCell>
                        <TableDataCell :status="'record'">{{ formatTanggal(data.date) }}</TableDataCell>
                        <TableDataCell :status="'record'">{{ data.supplier_id[0]['name'] }}</TableDataCell>
                        <TableDataCell :status="'record'">{{ calculateTotalPrice(data.listData) }}</TableDataCell>
                        <TableDataCell :status="'action'">
                            ...
                            <template v-if="hasPermission('inventory-purchase: update')">
                                <Link :href="route('inventoryPurchases.edit', data.id)" class="text-white mr-1 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-2 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3">
                    ...
                    <div class="relative w-full max-w-5xl max-h-full bg-white rounded-lg shadow dark:bg-gray-700">
                        <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                            <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                                TAMPILKAN PEMBELIAN PERSEDIAAN
                            </h3>
                        </div>
                        <div class="py-2">
                                                ...
                                                <table class="border-collapse border border-gray-400">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th class="border border-gray-300 px-2">Harga Barang</th>
                                                            <th class="border border-gray-300 px-2">Jumlah Barang</th>
                                                            <th class="border border-gray-300 px-2">Total Harga</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <tr v-for="(list, index) in form.listData" :key="list.id">
                                                            <td class="border border-gray-300 px-2 text-center">{{ index + 1 }}</td>
                                                            <td class="border border-gray-300 px-2">{{ list.product.product_name }}</td>
                                                            <td class="border border-gray-300 px-2">{{ formatRupiah(list.price) }}</td>
                                                            <td class="border border-gray-300 px-2 text-center">{{ list.quantity }}</td>
                                                            <td class="border border-gray-300 px-2">{{ formatRupiah(list.total_price) }}</td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </td>
