    ...
    <div class="max-w-[85rem] px-4 sm:px-6 lg:px-8 mx-auto my-4 sm:my-10">
        <div class="mb-5 pb-5 flex justify-between items-center border-b border-gray-200 dark:border-neutral-700">
            <div>
                ...
                <h2 class="text-2xl font-semibold text-gray-800 dark:text-neutral-200">Laporan Permintaan Return</h2>
            </div>
            ...
            <Table>
                ...
                <template #default>
                    <TableRow v-for="(data, index) in fetchData" :key="data.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <TableDataCell v-if="branch == 'SEMUA CABANG'" :status="'record'">{{ data.cabang }}</TableDataCell>
                        <TableDataCell :status="'record'">{{ formatDate(data.tanggal) }}</TableDataCell>
                        <TableDataCell :status="'record'">{{ data.nomor_ro }}</TableDataCell>
                        <TableDataCell :status="'record'">{{ data.nomor_return }}</TableDataCell>
                        <TableDataCell :status="'record'">{{ data.barang }}</TableDataCell>
                        <TableDataCell :status="'record'">{{ data.jumlah }}</TableDataCell>
                    </TableRow>
                </template>
            </Table>
        </div>
            ...
            <div class="w-full max-w-2xl sm:text-end space-y-2">
                <div class="grid grid-cols-2 sm:grid-cols-1 gap-3 sm:gap-2">
                    <dl class="grid sm:grid-cols-5 gap-x-3 text-sm">
                        <dt class="col-span-3 text-gray-500 text-xl">TOTAL PERMINTAAN :</dt>
                        <dd class="col-span-2 font-bold text-gray-800 text-2xl">{{ totalOmzet }} Barang</dd>
                    </dl>
                </div>
            </div>
        </div>
