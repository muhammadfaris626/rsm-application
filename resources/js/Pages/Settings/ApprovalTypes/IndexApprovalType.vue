        ...
        <div class="grid grid-cols-1 h-full space-y-6">
            ...
            <div class="bg-gradient-to-r from-slate-600 to-slate-700 rounded-xl shadow-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-2xl font-bold">Jenis Persetujuan</h1>
                        <p class="text-slate-100 mt-1">Kelola jenis persetujuan dan approval</p>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-md p-4">
                <div class="flex flex-col md:flex-row justify-between gap-4">
                    <div class="w-full md:w-1/3">
                        <div class="relative group">
                            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-gray-400 group-focus-within:text-slate-600 transition-colors">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <template v-if="hasPermission('approval-type: create')">
                            <button 
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                <Table>
                    <template #header>
                        <TableRow class="bg-slate-500">
                            <TableHeaderCell class="text-white">NO</TableHeaderCell>
                            <TableHeaderCell class="text-white">NAMA JENIS PERSETUJUAN</TableHeaderCell>
                            <TableHeaderCell class="text-white">AKSI</TableHeaderCell>
                        </TableRow>
                    </template>
                    <template #default>
                        <TableRow v-for="(data, index) in fetchData.data" :key="data.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-slate-50 dark:hover:bg-gray-600 transition-colors duration-150">
                            <TableDataCell :status="'number'">{{ index+1 }}</TableDataCell>
                            <TableDataCell :status="'record'">{{ data.approval_type_name }}</TableDataCell>
                            <TableDataCell :status="'action'">
                                <!-- Lihat Data  -->
                                <template v-if="hasPermission('approval-type: read')">
                                    <button 
                                ...
                                <!-- Ubah Data  -->
                                <template v-if="hasPermission('approval-type: update')">
                                    <button 
                                ...
                                <!-- Hapus Data  -->
                                <template v-if="hasPermission('approval-type: delete')">
                                    <button 
                ...
                <div class="relative w-full max-w-5xl max-h-full bg-white rounded-lg shadow dark:bg-gray-700">
                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                        <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                            TAMBAH JENIS PERSETUJUAN
                        </h3>
                    </div>
                    <form @submit.prevent="tambahData">
                        <div class="grid grid-cols-1 gap-2 px-4 py-2">
                            <div>
                                <InputLabel for="approval_type_name" value="Nama Jenis Persetujuan" />
                                <TextInput
                                    id="approval_type_name"
                                    type="text"
                                    class="mt-1 block w-full"
                                    v-model="form.approval_type_name"
                                />
                                <InputError class="mt-2" :message="form.errors.approval_type_name" />
                            </div>
                        </div>
                            ...
                            <table class="table-collapse w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                <tbody>
                                    ...
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <td class="px-6 py-4">
                                            {{ form.last_update && form.last_update.user ? form.last_update.user.name : '-' }}
                                        </td>
                                    </tr>
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                ...
                <div class="relative w-full max-w-5xl max-h-full bg-white rounded-lg shadow dark:bg-gray-700">
                    <div class="p-4 md:p-5 text-center">
                            ...
                            <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                                Apakah anda yakin ingin menghapus jenis persetujuan ini ?
                            </h3>
