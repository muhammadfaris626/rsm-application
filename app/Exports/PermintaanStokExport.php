...
class PermintaanStokExport implements FromCollection, WithEvents
{
    protected $data;
    ...
    public function collection()
    {
        ...
        $data = RequestOrder::query()
            ...
            ->get()
            ->flatMap(fn($order) => $order->listRequestOrder->map(fn($list, $index) => [
                'nomor' => $index + 1,
                'cabang' => $order->branch->branch_name,
                'tanggal' => $list->updated_at,
                'nomor_permintaan' => $order->ro_number,
                'kategori_barang' => $list->centerStock->product->productCategory->product_category_name,
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $headings = ['No', 'Cabang', 'Tanggal', 'Nomor Permintaan', 'Kategori Barang', 'Barang', 'Jumlah'];
