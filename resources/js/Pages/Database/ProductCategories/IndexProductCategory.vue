<script setup>
    ...
    const search = useDebouncedTableSearch('productCategories.index', usePage().props.search);
...
<template>
    ...
    <AuthenticatedLayout>
        <div class="grid grid-cols-1 h-full space-y-6">
            <!-- Header Card -->
            <div class="bg-gradient-to-r from-lime-600 to-lime-700 rounded-xl shadow-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-2xl font-bold">Kategori Barang</h1>
                        <p class="text-lime-100 mt-1">Kelola kategori produk dan barang</p>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-md p-4">
                <div class="flex flex-col md:flex-row justify-between gap-4">
                    <div class="w-full md:w-1/3">
                        <div class="relative group">
                            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-gray-400 group-focus-within:text-lime-600 transition-colors">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <template v-if="hasPermission('product-category: create')">
                            <button 
                                @click="downloadFile" 
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                <Table>
                    <template #header>
                        <TableRow class="bg-lime-500">
                            <TableHeaderCell class="text-white">NO</TableHeaderCell>
                            <TableHeaderCell class="text-white">KODE KATEGORI BARANG</TableHeaderCell>
                            <TableHeaderCell class="text-white">NAMA KATEGORI BARANG</TableHeaderCell>
                            <TableHeaderCell class="text-white">AKSI</TableHeaderCell>
                        </TableRow>
                    </template>
                    <template #default>
                        <TableRow v-for="(data, index) in fetchData.data" :key="data.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-lime-50 dark:hover:bg-gray-600 transition-colors duration-150">
                            <TableDataCell :status="'number'">{{ index+1 }}</TableDataCell>
                            <TableDataCell :status="'record'">{{ data.product_category_code }}</TableDataCell>
                            <TableDataCell :status="'record'">{{ data.product_category_name }}</TableDataCell>
                            <TableDataCell :status="'action'">
                                <template v-if="hasPermission('product-category: read')">
                                    <button 
                                ...
                                <template v-if="hasPermission('product-category: update')">
                                    <button 
                                ...
                                <template v-if="hasPermission('product-category: delete')">
                                    <button 
            ...
            <Modal :show="showModalCreate" @close="closeModalCreate">
                <div class="relative w-full max-w-5xl max-h-full bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <form @submit.prevent="tambahData">
                        <div class="grid grid-cols-2 gap-2 px-4 py-2">
                            <div>
                                <InputLabel for="product_category_code" value="Kode Kategori Barang" />
                                <TextInput
                                    ...
                                    disabled
                                />
                                <InputError class="mt-2" :message="form.errors.product_category_code" />
                            </div>
                            <div>
                                <InputLabel for="product_category_name" value="Nama Kategori Barang" />
                                <TextInput
                                ...
                                />
                                <InputError class="mt-2" :message="form.errors.product_category_name" />
                            </div>
                        </div>
                        <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
